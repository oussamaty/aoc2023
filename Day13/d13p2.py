...
def line_reflexion():
    ...
    scols, srows = [[] for k in range(len(segs))], [[] for k in range(len(segs))]
    for k in range(len(segs)):
        ...
        for j in range(len(segs[k][0]) - 1):
            ...
            if reflexion == 1:
                scols[k].append(j)
    return cols, rows, scols, srows
    
def full_reflexion():
    result = [0]*len(segs)
    cols, rows, scols, srows = line_reflexion()
    for k in range(len(scols)):
        if len(scols[k]) == 0: continue
        for col in scols[k]:
            reflexion = True
